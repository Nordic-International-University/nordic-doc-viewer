...
import { Loader2, X, Expand, Shrink, ChevronLeft, ChevronRight, Play, Pause } from 'lucide-react';
...
const PDFViewer: React.FC<PDFViewerProps> = ({ url, onClose }) => {
    ...
    const renderPage = useCallback(async (pageNum: number) => {
        if (!pdfDoc || !window.pdfjsLib || pageNum < 1 || pageNum > totalPages) return;
    ...
    const loadPDF = useCallback(async (fileUrl: string) => {
        ...
        try {
            ...
            setError('Не удалось загрузить PDF. Проверьте URL или CORS-политику сервера.');
            toast({
                title: "Xatolik",
                description: "PDF faylni yuklab bo'lmadi. URL manzilini yoki serverning CORS siyosatini tekshiring.",
                variant: "destructive",
            });
        } finally {
            setIsLoading(false);
    ...
    useEffect(() => {
        ...
        const handleKeyPress = (e: KeyboardEvent) => {
            switch(e.key) {
                ...
                case 'Escape': if (isPresentationMode) toggleFullscreen(); else onClose(); break;
    ...
    }, [isPresentationMode, goToPreviousPage, goToNextPage, toggleFullscreen, toggleSlideshow, onClose]);
    ...
    return (
        <>
            <style>{styles}</style>
            <div ref={presentationRef} className={`bg-background ${isPresentationMode ? 'fixed inset-0 z-50' : 'relative'}`}>
                <Card className={`w-full h-full flex flex-col ${isPresentationMode ? 'border-0 rounded-none' : ''}`}>
                    {!isPresentationMode && (
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button variant="ghost" size="icon" onClick={toggleFullscreen}>
                                        {isPresentationMode ? <Shrink className="h-5 w-5" /> : <Expand className="h-5 w-5" />}
                                    </Button>
                    ...
                    {!isPresentationMode && (
                        <CardFooter className="flex-shrink-0 flex items-center justify-center gap-4 p-4">
                            ...
                            <Button variant="outline" onClick={toggleSlideshow}>
                                {isSlideshow ? <Pause className="h-5 w-5 mr-2" /> : <Play className="h-5 w-5 mr-2" />}
